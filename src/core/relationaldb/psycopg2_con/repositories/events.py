...
async def get_event_by_id(conn: Connection, event_id: uuid.UUID) -> EventRead:
    query = """
        ...
        LIMIT 1
    """
    event_row = await conn.fetchrow(query, event_id)
    ...
    """
    participant_rows = await conn.fetch(participants_query, event_id)
    ...
    locations_query = """
        ...
        WHERE el.event_id = $1
    """
    location_rows = await conn.fetch(locations_query, event_id)
